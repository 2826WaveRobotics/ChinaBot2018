...
void Robot::DisabledPeriodic() {
	...
	std::cout << "Arm Angle = " << arm.get()->getCurrentAngle() << "   left encoder: "
			<< drivePID.get()->GetLeftEncoder() << "   right encoder: "
			<< drivePID.get()->GetRightEncoder()  << std::endl;
}
...
void Robot::TeleopPeriodic() {
	...
	std::cout << "Arm Angle = " << arm.get()->getCurrentAngle() <<std::endl;

	double leftJoystick = oi.get()->getDriver().get()->GetRawAxis(1);
	double rightJoystick = oi.get()->getDriver().get()->GetRawAxis(4);

	drivePID.get()->ArcadeDrive(leftJoystick, rightJoystick);

double leftJoystickO = oi.get()->getOperatorJS().get()->GetRawAxis(1);
arm.get()->moveArm(leftJoystickO);
